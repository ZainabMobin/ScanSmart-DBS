class employeeDatabase:
    ...
    def get_password_from_email(self, email):
        cursor = self.dbconn.cursor()
        query = "SELECT password FROM Employee WHERE Email = %s"
        cursor.execute(query, (email,))
    ...
    def check_contact_exists(self, ContactNumber):
        cursor = self.dbconn.cursor()
        query = "SELECT * FROM Employee WHERE ContactNumber = %s"
    ...
    def get_id_by_email(self, email):
        cursor = self.dbconn.cursor()
        query = "SELECT EmployeeID FROM Employee WHERE Email = %s"
        cursor.execute(query, (email,))
    ...
    def get_name_by_id(self, empid, role=None):
        cursor = self.dbconn.cursor()
        if role == "Cashier":
            query = "SELECT Name FROM Employee WHERE EmployeeID = %s AND Role = 'Cashier'"
        else:
