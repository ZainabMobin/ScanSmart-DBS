...
def show_display_bills_view():
    ...
    if "bill_page" not in st.session_state:
        st.session_state.bill_page = 1

    total_bills = len(employee_bills)
    total_pages = (total_bills + K_RESULTS - 1) // K_RESULTS

    # Clamp page safely
    st.session_state.bill_page = max(
        1, min(st.session_state.bill_page, total_pages)
    )

    # Slice bills for current page
    start = (st.session_state.bill_page - 1) * K_RESULTS
    end = start + K_RESULTS
    paged_bills = employee_bills[start:end]
    ...
    with bills_container:
        ...
        for bill in paged_bills:
            ...
            st.markdown(
                "<hr style='margin: 5px 0; border: 0.5px solid #f0f0f0;'>",
                unsafe_allow_html=True
            )
        ...
        with col1:
            if st.session_state.bill_page > 1:
        ...
        with col2:
            st.markdown(
                ...
                f"Page {st.session_state.bill_page} of {total_pages}"
                f"</p>",
                unsafe_allow_html=True
            )

        with col3:
            if st.session_state.bill_page < total_pages:
                if st.button("Next ➡"):
                    st.session_state.bill_page += 1
                    st.rerun()